...
[`map()`](https://purrr.tidyverse.org/reference/map.md) and its variants
with `in_parallel()`.

`in_parallel()` is a purrr adverb that plays two roles:

- It is a signal to purrr verbs like
  [`map()`](https://purrr.tidyverse.org/reference/map.md) to go ahead
...
[`map()`](https://purrr.tidyverse.org/reference/map.md) for usage
examples.
