...
Unlike [`map()`](https://purrr.tidyverse.org/reference/map.md) and its
variants which always return a fixed object type (list for
[`map()`](https://purrr.tidyverse.org/reference/map.md), integer vector
for [`map_int()`](https://purrr.tidyverse.org/reference/map.md), etc),
...
- `modify2()` modifies the elements of `.x` but also passes the elements
  of `.y` to `.f`, just like
  [`map2()`](https://purrr.tidyverse.org/reference/map2.md). `imodify()`
  passes the names or the indices to `.f` like
  [`imap()`](https://purrr.tidyverse.org/reference/imap.md) does.

- [`modify_in()`](https://purrr.tidyverse.org/reference/modify_in.md)
  modifies a single element in a
  [`pluck()`](https://purrr.tidyverse.org/reference/pluck.md) location.
