...
It's important to realise that this parallelism works by spreading computation across clean R sessions. That means that code like this will not work, because the worked daemons won't have a copy of `delay`:

```{r}
#| error: true
delay <- 0.5
out <- map(1:5, in_parallel(\(i) Sys.sleep(delay)))
```

You can resolve this by ensuring that your functions are self-contained, or pass any additional data in `in_parallel()`:

```{r}
out <- map(1:5, in_parallel(\(i) Sys.sleep(delay), delay = delay))
```
...
purrr provides a number of functions that work with predicate functions. Predicate functions take a vector and return either `TRUE` or `FALSE`, including functions like `is.character()` and `\(x) any(is.na(x))`. You typically use them to filter or find, and purrr comes with a bunch of helpers to make them easier to use:
