...
* Before R 4.1, anonymous functions were verbose, so we provide two convenient shorthands.
  For unary functions, `~ .x + 1` is equivalent to `function(.x) .x + 1`. This formula notation is only recommended if you require backward compatibility with R <= 4.1. We suggest using the shorthand notation where `\(x) x + 1` is equivalent to `function(x) x + 1`. 
