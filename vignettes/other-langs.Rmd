...
* Before R 4.1, anonymous functions were verbose, so we provide two convenient shorthands.
  For unary functions, `~ .x + 1` is equivalent to `function(.x) .x + 1`. This formula notation no longer recommended. We suggest using the shorthand notation in R >= 4.1 where `\(x) x + 1` is equivalent to `function(x) x + 1`. 
