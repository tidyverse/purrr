...
Atomic vector of desired type | 2 vectors | `NA` | `map2_lgl` (logical), `map2_int` (integer), `map2_dbl` (double), `map2_chr` (character), `map2_raw` (raw)
Atomic vector of desired type | >2 vectors | `NA` | `pmap_lgl` (logical), `pmap_int` (integer), `pmap_dbl` (double), `pmap_chr` (character), `pmap_raw` (raw)
...
Any | Vector and its names | `l/s/vapply(X, function(x) FUN(x, names(x)))` or `mapply/Map(FUN, X, names(X))` | `imap`, `imap_*` (`lgl`, `dbl`, `dfr`, and etc. just like in `map`, `map2`, and `pmap`)
...
Number of inputs | base R | `purrr`
:---:|--------|---------
1 | `function(x) f(x)` | `~ f(.x)`
2 | `function(x, y) f(x, y)` | `~ f(.x, .y)`
>2 | `function(x, y, z, ...) f(x, y, z, ...)` or `function(l) f(l[[1]], l[[2]], l[[3]], ...)` or `function(l) do.call(f, args = l)` | `~ f(..1, ..2, ..3, ...)`
