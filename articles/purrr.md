...
[`map()`](https://purrr.tidyverse.org/reference/map.md)[¹](#fn1)
...
[`map()`](https://purrr.tidyverse.org/reference/map.md): once you get
used to the syntax, it’s a very compact way to express the idea of
transforming a vector, returning one output element for each input
element. But there are several other reasons to use
[`map()`](https://purrr.tidyverse.org/reference/map.md), which we’ll
...
By default, [`map()`](https://purrr.tidyverse.org/reference/map.md) runs
only in your current R session. But you can easily opt in to spreading
your task across multiple R sessions, and hence multiple cores with
...
[`map()`](https://purrr.tidyverse.org/reference/map.md) call to run in
parallel:
...
[`map()`](https://purrr.tidyverse.org/reference/map.md) always returns a
list. But what if you want a different type of output? That’s where the
output variants come into play:

- There are four variants for the four most important types of atomic
  vector:

  - [`map_lgl()`](https://purrr.tidyverse.org/reference/map.md) returns
    a logical vector.
  - [`map_int()`](https://purrr.tidyverse.org/reference/map.md) returns
    an integer vector.
  - [`map_dbl()`](https://purrr.tidyverse.org/reference/map.md) returns
    a numeric (double) vector.
  - [`map_chr()`](https://purrr.tidyverse.org/reference/map.md) returns
    a character vector.

- For all other types of vector (like dates, date-times, factors, etc.),
  there’s [`map_vec()`](https://purrr.tidyverse.org/reference/map.md).
  It’s a little harder to precisely describe the output type, but if
  your function returns a length-1 vector of type “foo”, then the output
  of [`map_vec()`](https://purrr.tidyverse.org/reference/map.md) will be
  a length-n vector of type “foo”.

- [`modify()`](https://purrr.tidyverse.org/reference/modify.md) returns
  output with the same type as the input. For example, if the input is a
  data frame, the output will also be a data frame.

- [`walk()`](https://purrr.tidyverse.org/reference/map.md) returns the
  input (invisibly); it’s useful when you’re calling a function purely
  for its side effects, for example, generating plots or saving files.
...
[`map()`](https://purrr.tidyverse.org/reference/map.md) and friends all
iterate over a single list, making it poorly suited for some problems.
For example, how would you find a weighted mean when you have a list of
observations and a list of weights? Imagine we have the following data:
...
We could use [`map_dbl()`](https://purrr.tidyverse.org/reference/map.md)
to compute unweighted means:
...
[`map()`](https://purrr.tidyverse.org/reference/map.md) to compute a
...
[`map2()`](https://purrr.tidyverse.org/reference/map2.md):
[`pmap()`](https://purrr.tidyverse.org/reference/pmap.md) which can take
any number of varying arguments (passed as a list), and
[`imap()`](https://purrr.tidyverse.org/reference/imap.md) which iterates
over the values and indices of a single vector. Learn more in their
documentation.
...
1.  You might wonder why this function is called
    [`map()`](https://purrr.tidyverse.org/reference/map.md). What does
    it have to do with depicting physical features of land or sea 🗺? In
    fact, the meaning comes from mathematics where map refers to “an
    operation that associates each element of a given set with one or
    more elements of a second set”. This makes sense here because
    [`map()`](https://purrr.tidyverse.org/reference/map.md) defines a
    mapping from one vector to another. And “map” also has the nice
