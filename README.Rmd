...
* All `map()` functions accept functions (named, anonymous, and lambda), character vectors (used to extract components by name), or numeric vectors (used to extract by position). Note the use of the shorthand notation for anonymous functions (e.g. `\(x) x + 1` which is equivalent to `function(x) x + 1`). Formula notation for anonymous functions may also be used, but is no longer recommended; see note below.  
...
purrr functions translate formulas created by `~` into functions, e.g. `~ .x + 1` is equivalent to `function(.x) .x + 1`. This notation is only recommended if you require backward compatibility with R < 4.1.