...
* All `map()` functions accept functions (named, anonymous, and lambda), 
  ...
  (used to extract by position). Note the use of the shorthand notation for anonymous functions (e.g. `\(x) x + 1` which is equivalent to `function(x) x + 1`). Formula notation for anonymous functions (e.g. `~ .x + 1`) may also be used, but this is only recommended if you require backward compatibility with R <= 4.1.
